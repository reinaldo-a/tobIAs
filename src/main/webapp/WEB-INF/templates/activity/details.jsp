...
<div class="custom-container">
    <% if (activity != null) { %>
        ...
        <% if (isProfessor) { %>
            <div class="card border-0 shadow-sm p-4 mb-4">
                <h5 class="mb-3">Entregas dos Alunos</h5>
                        ...
                        <% for (ActivitySubmission submission : submissions) { %>
                            <div class="list-group-item px-0">
                                <div class="d-flex justify-content-between align-items-center gap-3">
                                    <div>
                                        <h6 class="mb-1"><%= submission.getStudentName() %></h6>
                                    ...
                                    <a href="${pageContext.request.contextPath}/Activity?action=submission&id=<%= submission.getId() %>" class="btn btn-sm btn-outline-primary">
                                        Ver respostas
                                    </a>
                                </div>
                            </div>
                ...
                <% } else { %>
                    <p class="text-muted text-center py-4 mb-0">Nenhum aluno enviou esta atividade ainda.</p>
                <% } %>
            </div>
        <% } %>
            ...
            <% if (questions != null && !questions.isEmpty()) { %>
                <% if (isStudent && !hasSubmission) { %>
                    <form action="${pageContext.request.contextPath}/Activity" method="post">
                        ...
                        <button type="submit" class="btn btn-primary mt-3">Enviar Atividade</button>
                    </form>
                <% } else { %>
                    <% if (isStudent && hasSubmission) { %>
